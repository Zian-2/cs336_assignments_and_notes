...
1 本文范围内的作业不需要用到GPU  
2 本课程在本章并未提供好系统适配，所以事实上不推荐在win上完成本章作业。在你的服务器or虚拟机上完成可以避免一些麻烦。
# Setup：
...
    # pstats：一个统计对象，专门用来格式化和读取cProfile产生的数据。
...
    # cumulative：按累积时间排序。如果train函数里调用了merge_step，那么merge_step运行的总时间都会算在train的累积时间里。这能帮你一眼看到“最慢的链路”。
    # 'tottime'：按函数自身耗时排序（不含调用子函数的时间），找到那些算法逻辑本身很重的函数。
    
...
windows上测试依旧出现问题。最好还是在linux上进行…
通过测试：
```python
============================= test session starts ==============================
